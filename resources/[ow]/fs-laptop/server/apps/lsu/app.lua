					...
					if
						...
						and (
							...
							or v.requireCurrency
								and v.coin ~= nil and Crypto:Has(source, v.coin, v.price)
						)
					then
						v.id = k
			...
			if char ~= nil and data and #data > 0 then
				...
				if not failed then
					...
					for k, v in ipairs(data) do
						...
						if
							(marketItem.rep == nil or Reputation:GetLevel(source, marketItem.rep) >= (marketItem.repLvl or 1))
							and (not marketItem.vpn or hasVpn)
							and (
								not marketItem.requireCurrency
								or marketItem.requireCurrency
									and marketItem.coin ~= nil
							)
						then
							if v.qty == -1 or v.qty >= v.quantity then
								if Crypto.Exchange:Remove(marketItem.coin, char:GetData("CryptoWallet"), math.floor(marketItem.price * v.quantity)) then
					...
					Laptop.Notification:Add(source, "Your Order", string.format("%s Item(s) Are Waiting for You at Today's Pickup Location. %s", boughtItemQuantity, extra), os.time() * 1000, 25000, "lsunderground", {}, {})
		...
		if plyr ~= nil then
			...
			if char ~= nil then
				...
				if pendingPickup then
					...
					Laptop.Notification:Add(source, "Your Order", "Thanks for collecting your order.", os.time() * 1000, 10000, "lsunderground", {}, {})
				else
