                ...
                elseif itemData then
                    if Inventory.Items:Has(char:GetData('SID'), 1, data.type, 1) then
                        if Inventory.Items:Remove(char:GetData('SID'), 1, data.type, 1) then
                            Wallet:Modify(targetChar:GetData('Source'), math.floor(itemData.price * math.random(0.50, 0.75)))
							Wallet:Modify(char:GetData('Source'), math.floor(itemData.price * math.random(0.70, 0.80)))
                            return cb(true)
                        end
                    end
                end
