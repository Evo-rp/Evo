...
local _raceItems = {
	...
	{ item = "lsundg_invite", coin = "VRM", price = 100, qty = -1, vpn = true },
...
function FinishRace(id)
	...
	if _races[id].completed ~= nil and #_races[id].completed > 0 then
        ...
        if (_races[id].total >= _reqForCrypto) and (_races[id].trackData.Type == "p2p" or tonumber(_races[id].laps) > 1) then
