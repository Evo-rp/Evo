...
AddEventHandler("Core:Shared:Ready", function()
	exports["evo-base"]:RequestDependencies("DJ", {
		"Callbacks",
		"Targeting",
		"Menu",
		"Notification",
...
RegisterNetEvent("evo-dj:client:playMusic", function(data)
	local booth = ""
	local boothId = 0
	...
	local p = promise.new()
	Callbacks:ServerCallback("evo-dj:server:songInfo", {}, function(cb)
		p:resolve(cb)
	end)
	...
	djMenu.Add:Input("Youtube URL", {
		...
		Callbacks:ServerCallback("evo-dj:server:playMusic", {
			song = song,
			zoneNum = boothId,
		}, function(success, zoneNum)
			if success then
				TriggerEvent("evo-dj:client:playMusic", { zone = zoneNum })
	...
	if xSound:soundExists(booth) then
		if xSound:isPlaying(booth) then
			djMenu.Add:Button("Pause Music", { success = true }, function()
				Callbacks:ServerCallback("evo-dj:server:pauseMusic", {
					zoneName = booth,
					zoneNum = boothId,
				}, function(success)
					if success then
		...
		elseif xSound:isPaused(booth) then
			djMenu.Add:Button("Resume Music", { success = true }, function()
				Callbacks:ServerCallback("evo-dj:server:resumeMusic", {
					zoneName = booth,
					zoneNum = boothId,
				}, function(success)
					if success then
		...
		djMenuSub["changevolume"].Add:Slider("Change Volume", {
			...
			Callbacks:ServerCallback("evo-dj:server:changeVolume", {
				volume = volume,
				zoneName = booth,
				zoneNum = boothId,
			}, function(success, zoneNum)
		...
		djMenu.Add:Button("Stop Music", { success = true }, function()
			Callbacks:ServerCallback("evo-dj:server:stopMusic", {
				zoneName = booth,
				zoneNum = boothId,
			}, function(success)
				if success then
