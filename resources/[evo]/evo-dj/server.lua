...
function RegisterCallbacks()
	Callbacks:RegisterServerCallback("evo-dj:server:songInfo", function(source, data, cb)
		cb(previousSongs)
	end)

	Callbacks:RegisterServerCallback("evo-dj:server:playMusic", function(source, data, cb)
		local src = source
	...
	Callbacks:RegisterServerCallback("evo-dj:server:changeVolume", function(source, data, cb)
		local src = source
		local zoneNum = data.zoneNum
	...
	Callbacks:RegisterServerCallback("evo-dj:server:stopMusic", function(source, data, cb)
		local src = source
		...
		TriggerClientEvent("evo-dj:client:playMusic", src, { zone = data.zoneNum })
	end)

	Callbacks:RegisterServerCallback("evo-dj:server:pauseMusic", function(source, data, cb)
		local src = source
		local zoneLabel = data.zoneName
		if Config.Locations[data.zoneNum].playing then
			Config.Locations[data.zoneNum].playing = nil
	...
	Callbacks:RegisterServerCallback("evo-dj:server:resumeMusic", function(source, data, cb)
		local src = source
		local zoneLabel = data.zoneName
...
-- -- This is for that
-- RegisterNetEvent("evo-dj:server:DestoryAll", function()
...
-- 	TriggerEvent("evo-dj:server:DestoryAll")
-- end)
