...
_G6 = {
	...
	Inventory = {
		...
		CreateJobPickup = function(self, group)
			Inventory.Items:RemoveAll(group, 3006, "group6_bag")
			Inventory:AddItem(group, 'group6_bag', 5, {}, 3006, nil, nil, nil, false, nil, nil)
		end,

		OpenJobPickup = function(self, playerSrc, group)
			Inventory:OpenSecondary(playerSrc, 3006, group, nil, nil, false, nil, nil, nil)
		end,

		DepositBag = function(self, playerSrc, VIN)
			local char = Fetch:Source(playerSrc):GetData("Character")
				...
				if count and count > 0 then
					...
					if Inventory.Items:Remove(char:GetData("SID"), 1, "group6_bag", count) then
						if VIN then
							...
							Inventory:AddItem(VIN, 'group6_bag', count, {}, 3007, nil, nil, nil, false, nil, nil)
		...
		OpenSafe = function(self, playerSrc, VIN)
			Inventory:OpenSecondary(playerSrc, 3007, VIN, nil, nil, false, nil, nil, nil)
		end,

		CleanUp = function(self, groupID, VIN)
			Inventory.Items:RemoveAll(groupID, 3006)
			Inventory.Items:RemoveAll(VIN, 3007)
		end
	}
}
...
AddEventHandler("Labor:Server:Startup", function()
	...
	Callbacks:RegisterServerCallback('Group6:DepositBag', function(source, data, cb)
	...
	Callbacks:RegisterServerCallback('Group6:OpenSafe', function(source, data, cb)
		if not data then
	...
	Callbacks:RegisterServerCallback('Group6:GrabBag', function(source, data, cb)
		if _joiners[source] and _group6[_joiners[source]].truck and _group6[_joiners[source]].state == 2 then
			...
			Execute:Client(source, "Notification", "Error", "You are not on the job")
			cb(false)
		end
	end)

	Callbacks:RegisterServerCallback("Group6:DespawnVehicle", function(source, data, cb)
	...
	RegisterNetEvent('Group6:Server:CrackSafe', function(netID)
		...
		if count and count > 0 then
			if Inventory.Items:Remove(char:GetData("SID"), 1, itemNeeded, 1) then
				...
				Callbacks:ClientCallback(source, "Group6:Client:CrackSafe", netID, function(success)
					if success then
				...
				Execute:Client(source, "Notification", "Error", "Failed to remove the item")
				return
			end
		else
			Execute:Client(source, "Notification", "Error", "You do not have the item")
			return
		end
	end)
end)
