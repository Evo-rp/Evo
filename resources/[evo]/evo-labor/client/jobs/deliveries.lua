...
AddEventHandler("Labor:Client:Setup", function()
	...
	Callbacks:RegisterClientCallback("Deliveries:DoingSomeAction", function(item, cb)
		local ped = PlayerPedId()
		if item == "GRAB_BOX" then
			Targeting.Zones:AddBox('DELIVERIES_DROP_OFF', "boxes-stacked", _route.Target.Coords, _route.Target.Data.First, _route.Target.Data.Second, {
...
RegisterNetEvent("Deliveries:Client:OnDuty", function(joiner, time)
	...
	_blip = Blips:Add("DeliveriesStart", "Deliveries Foreman", { x = 918.01, y = -1255.28, z = 0 }, 478, 3, 1.4)
	...
	eventHandlers["new-route"] = RegisterNetEvent(string.format("Deliveries:Client:%s:NewRoute", joiner), function(r)
		...
		if _blip ~= nil then
			Blips:Remove("DeliveriesStart")
			RemoveBlip(_blip)
			_blip = nil
		end
		
		_blip = Blips:Add("DeliveriesStart", "Delivery Location", { x = _route.coords.x, y = _route.coords.y, z = _route.coords.z }, 478, 3, 1.4)
	...
	eventHandlers["deliveries-grab"] = AddEventHandler("Deliveries:Client:Grab", function()
		Callbacks:ServerCallback("Deliveries:PackageGrab", {}, function(s)
			if s then
	...
	eventHandlers["spawn-truck"] = AddEventHandler("Deliveries:Client:DeliveriesSpawn", function()
		Callbacks:ServerCallback("Deliveries:DeliveriesSpawn", {}, function(netId)
	...
	eventHandlers["despawn-truck"] = AddEventHandler("Deliveries:Client:DeliveriesSpawnRemove", function()
		Callbacks:ServerCallback("Deliveries:DeliveriesSpawnRemove", {})
	end)
	...
	eventHandlers["turn-in"] = AddEventHandler("Deliveries:Client:TurnIn", function()
		Callbacks:ServerCallback("Deliveries:TurnIn", _joiner)
	end)
end)

AddEventHandler("Deliveries:Client:StartJob", function()
	Callbacks:ServerCallback("Deliveries:StartJob", _joiner, function(state)
		if not state then
