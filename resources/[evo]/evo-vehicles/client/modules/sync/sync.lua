...
AddEventHandler('Vehicles:Client:StartUp', function()
    AddTaskToVehicleThread('air', 1, true, function(veh, class)
        if not noDisableLeanInAir[class] and ((IsEntityInAir(SYNC_DRIVING_VEHICLE) and GetEntityHeightAboveGround(SYNC_DRIVING_VEHICLE) >= 0.8) or GetEntityRoll(SYNC_DRIVING_VEHICLE) < -60 or GetEntityRoll(SYNC_DRIVING_VEHICLE) > 60) then
